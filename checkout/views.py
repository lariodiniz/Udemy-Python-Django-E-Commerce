...
class PagSeguroView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        pg = order.pagseguro()
        pg.redirect_url = self.request.build.absolute_uri(
            reverse_lazy('checkout:order_detail', args=[order.pk])
        )
        pg.notification_url = self.request.build.obsolute_uri(
            reverse_lazy('checkout:pagseguro_notification')
        )
...
class PaypalView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        paypal_dict['return_url'] = self.request.build_absolute_uri(
            reverse_lazy('checkout:order_list')
        )
